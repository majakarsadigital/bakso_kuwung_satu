<tr class="bg-neutral-900 hover:bg-neutral-800/70 transition-colors duration-200">
    ...
    <td class="px-6 py-4">
        <span class="text-emerald-400 font-medium">
            {{ number_format($foodMenu->price, 0, ',', '.') }}
        </span>
    </td>
    ...
    <td class="px-6 py-4">
        <div class="flex items-center justify-center gap-2">
            ...
            <form action="{{ route('admin.menu.destroy', $foodMenu->id) }}" method="POST" class="inline"
    ...
    <div id="edit-modal-{{ $foodMenu->id }}" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 w-full max-w-2xl max-h-full">
            <div class="relative bg-neutral-900 border border-neutral-800 rounded-lg shadow-lg p-4 md:p-6">
                ...
                <div class="flex items-center justify-between border-b border-neutral-800 pb-4 md:pb-5">
                    ...
                    <button type="button" data-modal-hide="edit-modal-{{ $foodMenu->id }}"
                ...
                <form action="{{ route('admin.menu.update', $foodMenu->id) }}" method="POST"
                    ...
                    <div class="grid gap-4 py-4 md:py-6">
                        <div>
                            <label for="name-{{ $foodMenu->id }}"
                                class="block mb-2.5 text-sm font-medium text-neutral-300">Nama Menu</label>
                            <input type="text" name="name" id="name-{{ $foodMenu->id }}"
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" name="price" id="price-{{ $foodMenu->id }}"
                            ...
                            <div>
                                <label for="category-{{ $foodMenu->id }}"
                                    class="block mb-2.5 text-sm font-medium text-neutral-300">Kategori</label>
                                <input type="text" name="category" id="category-{{ $foodMenu->id }}"
                                    value="{{ old('category', $foodMenu->category) }}"
                                    class="bg-neutral-800 border border-neutral-700 text-white text-sm rounded-lg focus:ring-emerald-500 focus:border-emerald-500 block w-full px-3 py-2.5 shadow-xs placeholder:text-neutral-500"
                        ...
                        <div>
                            <label for="status-{{ $foodMenu->id }}"
                                class="block mb-2.5 text-sm font-medium text-neutral-300">Status</label>
                            <select name="is_available" id="status-{{ $foodMenu->id }}"
                                ...
                                <option value="1"
                                    {{ old('is_available', $foodMenu->is_available) == 1 ? 'selected' : '' }}>Tersedia
                                </option>
                                <option value="0"
                                    {{ old('is_available', $foodMenu->is_available) == 0 ? 'selected' : '' }}>Tidak
                                    Tersedia</option>
                            </select>
                        </div>

                        <div>
                            <label for="description-{{ $foodMenu->id }}"
                                class="block mb-2.5 text-sm font-medium text-neutral-300">Deskripsi Singkat</label>
                            <textarea name="short_description" id="description-{{ $foodMenu->id }}" rows="3"
                                ...
                                placeholder="Deskripsi singkat tentang menu">{{ old('short_description', $foodMenu->short_description) }}</textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="file" name="photo" id="photo-{{ $foodMenu->id }}" accept="image/*"
                    ...
                    <div class="flex items-center space-x-4 border-t border-neutral-800 pt-4 md:pt-6">
                        ...
                        <button data-modal-hide="edit-modal-{{ $foodMenu->id }}" type="button"
